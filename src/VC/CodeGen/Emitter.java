...
public final class Emitter implements Visitor {
  ...
  // PRE: ast must be a Program node--- travel from here
  ...
  public Object visitProgram(Program ast, Object o) {
    ...
    while (!list.isEmpty()) {
      DeclList dlAST = (DeclList) list;
      if (dlAST.D instanceof GlobalVarDecl) {
        GlobalVarDecl vAST = (GlobalVarDecl) dlAST.D; // Type Ident = Exp
    ...
    if (ast.I.spelling.equals("main")) {
      ...
      while (! fpl.isEmpty()) {
        if (((ParaList) fpl).P.T.equals(StdEnvironment.booleanType))
        	...
        	ArrayType arr_type = (ArrayType) ((ParaList) fpl).P.T;
  ...
  @Override
	public Object visitExprStmt(ExprStmt ast, Object o) {
		...
		if(!((ast.E instanceof CallExpr && ast.E.type.isVoidType()) || ast.E instanceof EmptyExpr || ast.E instanceof AssignExpr)) {
			emit(JVM.POP);
			frame.pop();
		}
		return null;
	...
	public Object visitIfStmt(IfStmt ast, Object o) {
		...
		// if(E)s1 else s2
		Frame frame = (Frame)o;
		if(ast.S2.isEmptyStmt()){//without else
	...
	public Object visitWhileStmt(WhileStmt ast, Object o) {
		// while(E){S} Also works when S is empty
		Frame frame = (Frame)o;
	...
	public Object visitForStmt(ForStmt ast, Object o) {
		// for(E1;E2;E3){Sa} How to deal with E1,E2,E3 empty??????
		Frame frame = (Frame) o;
		String L1 = frame.getNewLabel();
		String L2 = frame.getNewLabel();
		String L3 = frame.getNewLabel();
		//L1 begin of the loop, L2 end of the loop;
		frame.brkStack.push(L2);
		frame.conStack.push(L3);
		ast.E1.visit(this, o);

		if(!((ast.E1 instanceof CallExpr && ast.E1.type.isVoidType()) || ast.E1 instanceof EmptyExpr || ast.E1 instanceof AssignExpr)) {
			emit(JVM.POP);
			frame.pop();
		}
		emit(L1+":");
		...
		emit(L3+":");
		ast.E3.visit(this, o);
		if(!((ast.E3 instanceof CallExpr && ast.E3.type.isVoidType()) || ast.E3 instanceof EmptyExpr || ast.E3 instanceof AssignExpr)) {
			emit(JVM.POP);
			frame.pop();
		}
		emit(JVM.GOTO, L1);
	...
	*/
	     ...
	     if(ast.E.isEmptyExpr()){
	    	 ...
	    	 ast.E.visit(this, o);
	    	 if(ast.E.type.isFloatType()){ // returnn float
	    		 emit(JVM.FRETURN);
	    	 }else{ // return int and boolean
	    		 emit(JVM.IRETURN);
	    	 }
	     }
	     return null;
  ...
  public Object visitStringExpr(StringExpr ast, Object o) {
	...
	public Object visitBinaryExpr(BinaryExpr ast, Object o) {
		...
		if(op.equals("i&&")){
			...
			if(op.equals("i+")){
				...
				emit(JVM.FDIV);
				frame.pop();
			}else if(op.equals("i>") || op.equals("i<") || op.equals("i>=") || op.equals("i<=")
					|| op.equals("i==") ||op.equals("i!=")){
				emitIF_ICMPCOND(op, frame);

				//frame.pop();
			}else if(op.equals("f>") || op.equals("f<") || op.equals("f>=") || op.equals("f<=")
	...
	public Object visitAssignExpr(AssignExpr ast, Object o) {
		...
		if(ast.E1 instanceof ArrayExpr){
			//V[E] = E2; visit E first then E2 then istore_v
			...
			if(localvars.containsKey(((SimpleVar) arr.V).I.spelling+"[]")){
				if(((ArrayType )arr.V.type).T.isFloatType()){
					emit(JVM.FASTORE);
				}else if(((ArrayType )arr.V.type).T.isFloatType()){
					emit(JVM.BASTORE);
				}else{
					emit(JVM.IASTORE);
				}
			...
			//visit from right to Left E1 = E2; visit E2 first;
  ...
  public Object visitArrayType(ArrayType ast, Object o) {
  	...
  	// newarray pops a positive int, n, The new array is left on the stack
  	frame.pop(); 
  	frame.push();
  	return null;
  }
